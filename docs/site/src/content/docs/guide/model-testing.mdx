---
title: Testing authorization models
description: Run authorization-model tests against a hermetic, embedded OpenFGA server, with a disposable in-memory store for every run.
sidebar:
  order: 5
---
...
![The in-TUI model test workbench](../../../assets/model-test-tui.gif)
...
[`examples/model-tests/`](https://github.com/sergiught/openfga-cli/blob/main/examples/model-tests) and its README):
