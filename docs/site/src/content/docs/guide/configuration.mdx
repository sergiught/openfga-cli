---
title: Configuration
description: Where ofga stores its configuration, and how profiles, precedence, and environment variables work.
sidebar:
  order: 2
---
...
`ofga` decorates the TUI with small icons. Nerd Font glyphs look sharpest but
require a terminal configured with such a font — without one, the terminal
substitutes `?` for every icon.

No reliable way to detect a Nerd Font exists, so `auto` is deliberately
conservative: it starts from the universally-supported `unicode` set and only
upgrades on a positive signal (a recognised terminal, or a Nerd Font found in
your font directories). It never upgrades over SSH, because fonts installed on
the remote host say nothing about the terminal you are actually looking at.

When detection guesses `nerdfont` — the only rung that can render as boxes if
your font lacks coverage — the playground says so once on startup and points at
`ctrl+g`. You will not see that notice on the `unicode` or `off` rungs, which
always render, nor when you have set a value yourself.

To override the guess, press `ctrl+g` in the playground — it cycles `nerdfont` →
`unicode` → `off` and saves your choice. Or set it directly:

| Value | Behaviour |
| --- | --- |
| `auto` | Detect, falling back to `unicode` (default) |
| `nerdfont` | Always use Nerd Font glyphs |
| `unicode` | Widely-supported symbols only |
| `off` | No decorative glyphs; plain text labels |

```sh
OPENFGA_ICONS=unicode ofga
```

Pressing `ctrl+g` writes a concrete value and takes you out of `auto`. Set
`icons = "auto"` in your config file to restore detection.
