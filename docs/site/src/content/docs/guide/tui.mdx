...
- 🎯 **Two ways to narrow the tuples**: `/` searches the rows already loaded, while `f` sends a `/read` filter to the server — the only way to reach tuples past the 500 the pane loads. A server-side filter needs an object type: a whole type (`document:`), or one object (`document:roadmap`); a bare type on its own also needs a user.
