...
| `1`–`9` | Sidebar and panels (not in a text field or a dialog) | Jump straight to a section; `9` is Tests. While you're typing — a form, the DSL editor, an active `/` filter — a digit is just a digit, and dialogs keep their own keys |
...
| `ctrl+g` | Global | Cycle glyph styles: nerdfont / unicode / off (always active, even inside overlays and forms) |
